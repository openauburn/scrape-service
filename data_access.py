...
def publish(endpoint, body):
    return requests.post(urljoin(da_url, endpoint), json=body, headers={'OA-Token': token.strip()})
...
def publish_data(dataset, data, r):
    for i in range(0,len(data), STEP): 
        ...
        if resp.status_code != 200:
            r.errors.append({'message': f"publishing: {resp.status_code}", 'message_ext': f"publishing: {resp.json()}"})
    return
...
def get_latest_datum(dataset):
    resp = requests.get(urljoin(da_url, f"datasets/{dataset}?sort=_id,desc"))
    return resp.json()['data'][0]


def log_scrape_job(dataset, started_at, ended_at, data_count, data_errors, total_errors):
    resp = publish("/scrape_jobs", [{
        'dataset': dataset,
