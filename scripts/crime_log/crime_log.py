...
def scrape(latest, r):
    ...
    for row_i in tqdm(range(len(rows))):  # For each incident
        row = rows[row_i]
        fields = row.find_elements(By.TAG_NAME, "td")
        ...
        address, latitude, longitude = geocode.get_lat_long(
            fields[10].text + " Auburn Alabama", r
        )
        ...
        record = {
            ...
            "occurred_from": safety_format.datetime_format(
                fields[3].text, fields[4].text, r
            ),
            "occurred_to": safety_format.datetime_format(
                fields[5].text, fields[6].text, r
            ),
