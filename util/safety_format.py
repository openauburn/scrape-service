...
def string_format(string, r):
    if string == " " or string == "NA" or len(string.strip()) == 0:
        return None
    else:
        return string.strip()
...
def time_format(hh_mm, r):
    try:
        ...
        r.errors.append(
            {
                "message": f"Failure while processing time {hh_mm}",
                "message_ext": str(err),
                "occurred_at": datetime.strftime(datetime.now(), err_dt),
            }
        )
...
def integer_format(maybe_int, r):
    if maybe_int == "" or maybe_int == " ":
        return 0
    else:
        return int("".join([c for c in maybe_int if c.isdigit()]))
